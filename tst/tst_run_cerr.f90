!*******************************************************************************
!Program - tst_run_cerr
!*******************************************************************************
program tst_run_cerr

!Purpose:
!This Fortran program allows comparing the error estimates in two netCDF files
!from RAPID simulations.
...
!netCDF file ids
integer, parameter :: IS_nc_ndim_err=2
!number of dimensions
integer, dimension(IS_nc_ndim_err) :: IV_nc_start_err,IV_nc_count_err
...
integer :: IS_nc_id_var_Qerr_1, IS_nc_id_var_rivid_1,                          &
           IS_nc_id_var_Qerr_2, IS_nc_id_var_rivid_2
!netCDF variable ids
...
integer :: IS_err, JS_err
double precision :: ZS_rdif,ZS_rdif_max
double precision :: ZS_adif,ZS_adif_max

character(len=120) :: temp_char

double precision, allocatable, dimension(:) :: ZV_Qerr_1
double precision, allocatable, dimension(:) :: ZV_Qerr_2
double precision, allocatable, dimension(:) :: ZV_dQerr_abs
...
IS_nc_status=NF90_INQ_VARID(IS_nc_id_fil_Qout_1, 'Qout_err',IS_nc_id_var_Qerr_1)
call nc_check_status(IS_nc_status)
...
IS_nc_status=NF90_INQ_VARID(IS_nc_id_fil_Qout_2, 'Qout_err',IS_nc_id_var_Qerr_2)
call nc_check_status(IS_nc_status)
...
!Allocate variables
!*******************************************************************************
allocate(ZV_Qerr_1(IS_riv_bas))
allocate(ZV_Qerr_2(IS_riv_bas))
allocate(ZV_dQerr_abs(IS_riv_bas))
...
!*******************************************************************************
!Checking that Qout_err variables are the same 
!*******************************************************************************
IV_nc_start_err = (/1,1/)
IV_nc_count_err = (/IS_riv_bas,1/)
IS_err=3
...
!Reading values
!-------------------------------------------------------------------------------
IS_nc_status=NF90_GET_VAR(IS_nc_id_fil_Qout_1,IS_nc_id_var_Qerr_1,             &
                          ZV_Qerr_1,IV_nc_start_err,IV_nc_count_err)
call nc_check_status(IS_nc_status)

IS_nc_status=NF90_GET_VAR(IS_nc_id_fil_Qout_2,IS_nc_id_var_Qerr_2,             &
                          ZV_Qerr_2,IV_nc_start_err,IV_nc_count_err)
call nc_check_status(IS_nc_status)
...
!-------------------------------------------------------------------------------
do JS_riv_bas=1,IS_riv_bas
     ZV_dQerr_abs(JS_riv_bas)=abs(ZV_Qerr_1(JS_riv_bas)-ZV_Qerr_2(JS_riv_bas))
end do

ZS_rdif=sqrt(sum(ZV_dQerr_abs*ZV_dQerr_abs)/sum(ZV_Qerr_1*ZV_Qerr_1))
if (ZS_rdif>ZS_rdif_max) ZS_rdif_max=ZS_rdif

do JS_riv_bas=1,IS_riv_bas
     ZS_adif=ZV_dQerr_abs(JS_riv_bas)
...
!Ending loop
!-------------------------------------------------------------------------------
IV_nc_start_err(2) = IV_nc_start_err(2) + 1
end do
