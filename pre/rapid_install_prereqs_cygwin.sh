...
# $ chmod u+x rapid_install_prereqs_cygwin.sh 
#
#Compilers for C, C++ and FORTRAN are needed in order to install the libraries 
#used by RAPID. Here we use the GNU Compiler Collection. Make sure all necessary 
...
cd petsc-3.6.2
./configure PETSC_DIR=$PWD PETSC_ARCH=linux-gcc-cxx --download-fblaslapack=1 --download-mpich=1 --with-cc=gcc --with-cxx=g++ --with-fc=gfortran --with-clanguage=cxx --with-debugging=0 --with-windows-graphics=0
